...
class arguments():
    def __init__(self):
        ...
        self.dataset = 'ycb'
        # self.dataset_root = '/home/user/PycharmProjects/DenseFusion/datasets/linemod/Linemod_preprocessed'
        # self.dataset_root = '/media/user/ssd_1TB/YCB_dataset'
        self.dataset_root = '../YCB_dataset'
        self.num_objects = 13
        
        self.flag_pretrained_vgg = False
        # self.flag_pretrained = True
        self.flag_pretrained = False
        self.path_pretrained = 'checkpoint/YCBDataset/checkpoint_65Epoch.pth.tar'
#         self.num_pretrain_param = 36
        ...
        self.vertex_reg = False
        self.vertex_reg_hough = False
        ...
        self.labels = ["ape", "bench", "camera", "can", "cat", "driller", "duck", "eggbox", "glue", "punch", "iron", "lamp", "phone"]
...
def train(trainloader, net, criterion, criterion_center, optimizer, device, device_cpu, start_epoch):
    global opt
    loss_his = []
    images = None
    labels = None
    ...
    bboxs_gt = None
    for epoch in range(start_epoch, opt.nepoch): #TODO decide epochs
        ...
        for i, data in enumerate(trainloader):
            ...
            if opt.train_single_frame:
                ...
                if opt.vertex_reg == True:
                  ...
                  output_seg, output_center_dir = net(images, extents, meta, gt_hough, 1, device_cpu)
#                   print("This is the output of hough voting: ", output_hough.cpu().numpy())
                  ...
                  print("Iter " + str(i) + "  //  state_dict : " + str(net.state_dict()))
                else:
                    # Only train the segmentation part
                    images, labels = data

                    # image_mat = images[1].cuda().cpu().clone().numpy()
                    # image_mat = np.transpose(image_mat, (1, 2, 0))
                    # # print(image_mat)
                    # image_mat = image_mat.astype(np.float32)
                    # image_mat /= 255.0
                    # image_mat0 = image_mat[:, :, 0].copy()
                    # image_mat[:, :, 0] = image_mat[:, :, 2]
                    # image_mat[:, :, 2] = image_mat0
                    ...
                    # print("output_seg typq : " + str(type(output_seg)))
                    # print("output_seg shape : " + str(output_seg.shape))
                    ...
                    # num = []
                    # labels_mat = labels[1].cuda().cpu().clone().numpy()
                    # # for label in labels_mat:
                    # s1, s2 = labels_mat.shape
                    # for j in range(s1):
                    #     for k in range(s2):
                    #         if labels_mat[j][k] not in num:
                    #             num.append(labels_mat[j][k])
                    # print(num)
                    # cv2.imshow("image_mat", image_mat)
                    # cv2.waitKey(0)

                    # label_mat = labels[1].cuda().cpu().clone().numpy()
                    # label_mat.resize((480, 640, 1))
                    # label_mat = label_mat.astype(np.uint8)
                    # cv2.imshow("labels", label_mat)
                    # cv2.waitKey(0)
            else:
            ...
            #
            #     # print(" --- loss : " + str(loss_his[int(10 * (epoch + 1) - 1)]))
            #     save_checkpoint({
            #         'epoch': opt.nepoch,
            #         'arch': opt.arch,
            #         'state_dict': net.state_dict(),
            #         'test_loss': loss_his[len(loss_his)-1],
            #         'dataset': "YCB",
            #         'optimizer': optimizer.state_dict(),
            #     }, False, filename="checkpoint/checkpoint_" + str(epoch + 1) + "Epoch_" + str(i) + "iter.pth.tar")
        ...
        save_checkpoint({
            ...
            'iter': iter,
            'state_dict': net.state_dict(),
            # 'test_loss': loss_his[int(10 * (epoch+1) - 1)],
            'test_loss': loss_his[len(loss_his)-1],
            'dataset': "YCB",
            'optimizer': optimizer.state_dict(),
        }, False, filename="checkpoint/YCBDataset/checkpoint_" + str(epoch + 1) + "Epoch.pth.tar")
        ...
        for data in tqdm(testloader):
            if opt.train_single_frame:
                if opt.vertex_reg == True:
                    ...
                    output_seg, output_center_dir = net(images, extents, meta, gt_hough, 0, device_cpu)
                    ...
                    image_mat = images[0].cuda().cpu().clone().numpy()
                    # print("out image shape : " + str(image_mat.shape))
                    image_mat = np.transpose(image_mat, (1, 2, 0))
                    image_mat = image_mat.astype(np.float32)
                    image_mat /= 255.0
                    image_mat0 = image_mat[:, :, 0].copy()
                    ...
                    labels = labels.type('torch.LongTensor').to(device)
                    output_seg = net(images)
                    # print("output_seg type : " + str(type(output_seg)))
                    # print("output_seg shape : " + str(output_seg.shape))
                    loss_temp = criterion(output_seg, labels)
                    # print(str(loss_temp))
                    # print("loss temp type : " + str(type(loss_temp)))
                    # print("loss temp.item() : " + str(loss_temp.item()))
                    # print("loss temp.item() shape : " + str(loss_temp.item().shape))
                    loss += loss_temp.item()

                    label_mat = labels[0].cuda().cpu().clone().numpy()
                    label_mat.resize((480, 640, 1))
                    label_mat = label_mat.astype(np.uint8)
                    print("label shape : " + str(label_mat.shape))
                    # label_mat = np.transpose(label_mat, (1, 2, 0))

                    cv2.imshow("labels", label_mat)
                    cv2.waitKey(0)

                    # output_mat = output_seg.cuda().cpu().clone().numpy()
                    # print(output_mat)
                    # output_mat = output_mat.astype(np.float32)
                    # output_mat.resize((480, 640, 1))
                    # cv2.imshow("output", output_mat)
                    # cv2.waitKey(0)

                    index = 0
                    for output in output_seg[0]:
                        output_mat = output.cuda().cpu().clone().numpy()
                        output_mat.resize((480, 640, 1))
                        # output_mat *= 255
                        output_mat = output_mat.astype(np.float32)
                        # output_mat = output_mat.astype(np.uint8)
                        max_val = 0
                        # print("output mat shape : " + str(output_mat.shape))
                        row, col, _ = output_mat.shape
                        for i in range(row):
                            for j in range(col):
                                if output_mat[i][j] > max_val:
                                    max_val = output_mat[i][j]
                        print("Max value : " + str(max_val))
                        # print(output_mat)
                        cv2.imshow("output"+str(index), output_mat)
                        cv2.waitKey(0)
                        index += 1
            ...
            #     cnt_image+=1
            #     save_image(images, labels, output_seg, vertex_targets, output_center_dir, 0, 'test', cnt, cnt_image)
...
def main():
    ...
    # print("dataset cld : " + str(dataset.cld))
    # return

    if opt.mode == "eval":
        if opt.dataset == 'ycb':
            dataset = PoseDataset_ycb('test', opt.num_points, True, opt.dataset_root,
                                      opt.noise_trans, opt.refine_start)
            # print("No YCB dataset")
            # return
            ...
            dataset = PoseDataset_linemod('test', opt.num_points, False, opt.dataset_root,
                                               0.0, opt.refine_start,
...
#     criterion = nn.MSELoss()
    ...
    if opt.flag_pretrained and not opt.flag_pretrained_vgg:
        ...
        if os.path.isfile(opt.path_pretrained):
            ...
            pretrained_list = list(pretrained_dic.items())
            # print("pretrained list length : " + str(len(pretrained_list)))
#            net.load_state_dict()
            start_epoch = pre_trained['epoch']
            if opt.num_pretrain_param_load > 0:
                ...
                for k, v in net_dic.items():
                    ...
                    count+=1
                    # print("net dic new k : " + str(net_dic_new[k]))
    ...
    if opt.mode == "train":
        loss_his = []
        loss_his = train(trainloader, net, criterion, criterion_center, optimizer, device, device_cpu, start_epoch)
